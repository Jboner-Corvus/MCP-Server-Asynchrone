...
export function initQueues(
  ...
  const deadLetterQueue = new Queue<AsyncTaskJobPayload, unknown, string>(DEAD_LETTER_QUEUE_NAME, {
    connection: redisConnection,
    defaultJobOptions: {
      attempts: 1,
  ...
  return { taskQueue, deadLetterQueue, redisConnection, taskQueueEvents, deadLetterQueueEvents };
}
